...
//Dibuja el calendario del mes y año que indique
function aceptar(){
    deleteMesPpal()
...
//pintar con color rojo el dia cuya fecha tenga un evento asociado
function pintarEvento(i){
    if (localStorage.getItem("count")){
        ...
        for(elements of EventGenerales){
                ...
                console.log(FechasEvents)
                for(let k=0;k<FechasEvents.length;k++){
                    PrimeraFecha=FechasEvents[k] 
                    console.log(k)
...
function DibujarEventoEnPantalla(){
    ...
    for (let elements of EventGenerales){
        ...
        evento.innerHTML+=`<div class="list-group list-group-horizontal" id=${elements.id.substr(5,1)}> <li class="list-group-item col-3">${elements.fecha}  </li>
